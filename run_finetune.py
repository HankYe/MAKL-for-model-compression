...
setproctitle.setproctitle("yhc:FitNet")
num_cls = 21
Batch_size = 32
Feature_num = 6
Epoch = 250
...
def update_vis_plot(iteration, loc_s, conf_s, feat_loss, window1, update_type, epoch_size=1):
    if iteration != 0:
        viz.line(
            X=torch.ones((1, 4)).cpu() * iteration,
            Y=torch.Tensor([loc_s, conf_s, loc_s + conf_s, feat_loss]).unsqueeze(0).cpu() / epoch_size,
            win=window1,
            update=update_type
        )
    else:
...
if __name__ == '__main__':
    ...
    T_file = 'model/Finetune/prune_' + repr(compression_rate) + '/' + repr(Epoch) + 'epoch/lr_150190230/20200509/'
    ...
    criterion_d = MultiBoxLoss(21, 0.5, True, 0, True, 3, 0.5, False, True).cuda()
    ...
    criterion_hint = HintLoss().cuda().to(device)
    ...
    while rate > 0.5:
        ...
        vis_title = 'Finetune compression(10%) rate: ' + str(rate)
        vis_legend = ['Loc S Loss', 'Conf S Loss', 'Total S Loss', 'Feat Loss']
        vis_legend_test = ['Loc Loss', 'Conf Loss', 'Total Loss']
        vis_title_test = 'Finetune test(10%) loss on 07test'
        ...
        for batch in range(epoch_test):
            ...
            with torch.no_grad():
                ...
                loc_test, cls_test = criterion_d(output, targets, device)
        ...
        model = FitNet(T_model, S_model, Feature_num, use_gan=False).cuda().to(device)
        ...
        for epoch in range(0, Epoch):
            ...
            for batch in range(epoch_size):
                ...
                T_feature, feat_S, out_S, _ = model(images)

                loss_feat = criterion_hint(feat_S, T_feature)
                loss_l, loss_c= criterion_d(out_S, targets, device)

                feat_loss += loss_feat.data.item()
                loc_loss += loss_l.data.item()
                cls_loss += loss_c.data.item()
                S_loss = loss_c + loss_l
                ...
                update_vis_plot(j, loss_l.data.item(), loss_c.data.item(), loss_feat.data.item(), iter_plot, 'append')

                j += 1
            if epoch % 10 == 9:
                torch.save(S_model, T_file + 'Finetune_ssd300_VOC_' + repr(iteration) + '_Epoch' + str(
                    epoch + 1) + '.pkl')
            ...
            for batch in range(epoch_test):
                ...
                with torch.no_grad():
                    ...
                    _, output = S_model(images)
                    loc_test, cls_test = criterion_d(output, targets, 'cuda:1')
                    loc_test_total += loc_test.data.item()
                    cls_test_total += cls_test.data.item()
            ...
            del batch_test
            S_model.to(device)
            update_vis_plot(epoch, loc_loss, cls_loss, feat_loss, epoch_plot, 'append', epoch_size)
