...
if __name__ == '__main__':
    ...
    T_file = 'model/KD/T_const/prune_list/' + repr(Epoch) + 'epoch/lr_153040/20200407/'
    ...
    criterion_kd = KDloss(21, temperature=6.0, alpha=0.95).cuda()
    ...
    while rate > 0.5:
        ...
        vis_title = 'KD compression rate: ' + str(rate)
        vis_legend = ['Loc KD Loss', 'Conf KD Loss', 'Total KD Loss', 'Loc S Loss', 'Conf S Loss', 'Total S Loss']
        ...
        for epoch in range(Epoch):
            loc_loss = cls_loss = loc_loss_kd = cls_loss_kd = 0
            ...
            for batch in range(epoch_size):
                ...
                _, out_T = T_model(images)
                _, out_S = S_model(images)
                ...
                print("[Epoch %d/%d][Batch %d/%d][S_loss: %f]" % (epoch + 1, Epoch, i, epoch_size, S_loss))

                update_vis_plot(j, loss_l.data.item(), loss_c.data.item(), loss_l_s.data.item(), loss_c_s.data.item(),
                                iter_plot, 'append')

                j += 1
            if epoch % 5 == 4:
                torch.save(S_model, T_file + 'kd_ssd300_VOC_' + repr(iteration) + '_Epoch' + str(
                    epoch + 1) + '.pkl')

            update_vis_plot(epoch, loc_loss_kd, cls_loss_kd, loc_loss, cls_loss, epoch_plot, 'append', epoch_size)
