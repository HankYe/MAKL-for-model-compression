...
setproctitle.setproctitle("yhc:KDGAN-multiD-TV")
num_cls = 21
Batch_size = 16
Channel = 64
Feature_num = 2
...
if __name__ == '__main__':
    ...
    T_file = 'model/KDGAN_' + repr(Feature_num) + 'F' + repr(Feature_num) + 'D/T_var/prune_list/' + repr(
        Epoch) + 'epoch/lr_05101520/20200328/'
    if os.path.exists(Path(T_file)) is False:
        os.makedirs(T_file)

    priorbox = PriorBox(voc, device).to(device)
    # priors = torch.autograd.Variable(priorbox())
    ...
    while rate > 0.5:
        ...
        # criterion_rank = nn.DataParallel(criterion_rank, device_ids=device_ids)
        ...
        optimizer_D_dis = torch.optim.Adam([{'params': d.dis.parameters()} for d in model.module.dis], lr=0.00002, betas=(0.5, 0.999))
        optimizer_D = torch.optim.Adam([{'params': d.conv1.parameters()} for d in model.module.dis] +
                                        [{'params': d.conv2.parameters()} for d in model.module.dis] +
                                        [{'params': d.conv3.parameters()} for d in model.module.dis], lr=0.001, betas=(0.5, 0.999))
        optimizer_S = torch.optim.SGD(S_model.parameters(), lr=1e-3, momentum=0.9, weight_decay=5e-4)
        optimizer_D_OD = torch.optim.SGD([{'params': d.cls.parameters()} for d in model.module.dis] +
                                      [{'params': d.offset.parameters()} for d in model.module.dis], lr=1e-3, momentum=0.9, weight_decay=5e-4)
        ...
        for epoch in range(Epoch):
            ...
            for batch in range(epoch_size):
                ...
                d_T, d_S, outT_test, out_S = model(images)
            ...
            if epoch % 5 == 4:
                torch.save(S_model, T_file + 'ssd300_VOC_C' + str(Channel) + '_' + repr(iteration) + '_Epoch' + str(epoch + 1) + '.pkl')
                torch.save(KDGAN_Feat, T_file + 'KDGAN_VOC_C' + str(Channel) + '_' + repr(iteration) + '_Epoch' + str(epoch + 1) + '.pkl')
