...
if __name__ == '__main__':
    ...
    T_file = 'model/FitNet/prune_' + repr(compression_rate) + '/' + repr(Epoch) + 'epoch/lr_150190230/20200429/T_var_alpha_var/'
    ...
    while rate > 0.5:
        ...
        for batch in range(epochT):
            ...
            with torch.no_grad():
                ...
                loc, cls = criterion_d(output, targets, device)
        ...
        criterion_kd = KDloss(21, temperature=2.0, minlossT=minloss, maxlossT=maxloss).cuda()
        ...
        vis_title = 'FitNet compression (temp2.0, 10%) rate: ' + str(rate)
        ...
        vis_title_test = 'FitNet test(2.0, 10%) loss on 07test'
        iter_plot = create_vis_plot('Iteration', 'Loss(T-const)', vis_title, vis_legend)
        epoch_plot = create_vis_plot('Epoch', 'Loss(T-const)', vis_title, vis_legend, start=0)
        ...
        for epoch in range(0, Epoch):
            ...
            for batch in range(epoch_size):
                ...
                T_feature, feat_S, out_S, out_T = model(images)

                loss_feat = criterion_hint(feat_S, T_feature)
                loss_l, loss_c, loss_l_s, loss_c_s = criterion_kd(out_S, out_T, targets)

                feat_loss += loss_feat.data.item()
                loc_loss += loss_l_s.data.item()
                cls_loss += loss_c_s.data.item()
                loc_loss_kd += loss_c.data.item()
                cls_loss_kd += loss_l.data.item()
                ...
                j += 1
            if epoch % 5 == 4:
                torch.save(S_model, T_file + 'FitNet_ssd300_VOC_' + repr(iteration) + '_Epoch' + str(
                    epoch + 1) + '.pkl')
